create or replace function wmeter.user_password_partition() returns trigger as
$$
declare
    _schema         varchar(64)  := 'wmeter';
    _table          varchar(64)  := 'user_password';
    _offset         int;
    _low            varchar(32);
    _high           varchar(32);
    _max            int          := 100000;
...
begin
    ...
    if NOT FOUND then
        ...
        EXECUTE format('CREATE INDEX %I ON %s.%I (user_id, valid_from)',
            _tablePartition || '_USER_PWD_IDX',
            _schema,
            _tablePartition);
